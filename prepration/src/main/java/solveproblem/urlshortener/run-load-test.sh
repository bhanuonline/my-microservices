...
# Runs k6 load test + captures Docker container CPU/Memory
# stats in the background, then APPENDS a combined, timestamped
# report to a persistent log file (report never overwritten).
# ============================================================

STATS_FILE="docker_stats_snapshot_temp.log"   # temporary, overwritten each run
REPORT_FILE="loadtest_history.log"            # permanent, appended every run
K6_SCRIPT="loadtest-write.js"
...

{
  echo ""
  echo "################################################################"
  echo "# TEST RUN AT: $RUN_TIMESTAMP"
  echo "################################################################"
  echo "$K6_OUTPUT"
  echo ""
...
echo "(Open that file to see this run alongside all previous runs, each timestamped.)"